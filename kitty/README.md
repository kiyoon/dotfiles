...
[finder-quick-actions.sh](finder-quick-actions.sh) installs two entries into the
Finder right-click menu:

- **Open in kitty** — a new OS window at that folder
- **Open in kitty tab** — a new tab in the kitty window you are already using

```sh
./finder-quick-actions.sh              # into ~/Library/Services
./finder-quick-actions.sh --uninstall
```

A folder opens itself, a file opens its parent folder, and right-clicking the
empty background of a Finder window opens that window's folder. Selecting
several items opens one window or tab each.

Both entries are Automator `.workflow` bundles, generated by the script rather
than committed, because a bundle symlinked into `~/Library/Services` is not
guaranteed to register with the Services scanner. Each bundle is a shim that
runs [open-in-kitty.sh](open-in-kitty.sh), so behaviour changes go in that
script and need no reinstall. The install prints a `pbs -flush`; without it the
entries can take a login to appear.

[open-in-kitty.sh](open-in-kitty.sh) drives the running kitty over the remote
control socket described above. `kitty --single-instance` looks like the simpler
choice and is the wrong one: new windows go to the first instance *started with
that flag*, and kitty launched from the Dock was not, so a quick action using it
would open a second instance and leave the one you are looking at untouched.

Socket discovery cannot pick the newest `/tmp/kitty-<pid>`. kitty does not
remove the socket when it exits, so `/tmp` accumulates sockets from dead
instances; the script keeps only those whose pid still answers `kill -0`, and
takes the newest of those. When none are live, kitty is not running and it opens
a new instance at the folder instead.
