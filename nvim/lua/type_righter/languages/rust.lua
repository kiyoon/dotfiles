...
local function get_node_that_covers_type()
  ...
  if node == nil then
    notify.notify("Not in a type node", vim.log.levels.ERROR, { title = "toggle_option_type" })
    return nil
  end
...
---Return the @type node from `type_righter.scm` query.
...
local function get_type_node_under_cursor(node)
  ...
  local query = vim.treesitter.query.get("rust", "type_righter") ---@type vim.treesitter.Query?
  if query == nil then
    notify.notify("Query not found.", vim.log.levels.ERROR, { title = "toggle_option_type" })
    return nil
  end
  ...
  require("type_righter.notify").notify(
    "Cursor not in a type node.",
    vim.log.levels.ERROR,
    { title = "toggle_option_type" }
  )
  return nil
end
...
---Adds `Result<...>` to the type hint (or strip it).
M.toggle_result_type = function()
  utils.make_dot_repeatable(function()
    ...
    if text:match("^Result%<.*%>$") then
      ...
      if type_node == nil then
        notify.notify("Unknown error", vim.log.levels.ERROR, { title = "toggle_option_type" })
        return
      end
