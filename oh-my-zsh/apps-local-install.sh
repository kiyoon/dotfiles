...
if [[ $OSTYPE == "darwin"* ]]; then
  ...
  if ! command -v pv &>/dev/null; then
    TEMPDIR=$(mktemp -d)
    cd "$TEMPDIR" || { echo "Failure"; exit 1; }
    wget https://codeberg.org/ivarch/pv/releases/download/v1.9.34/pv-1.9.34.tar.gz
    # wget https://ivarch.com/s/pv-1.9.34.tar.gz
    tar xzf pv-1.9.34.tar.gz
    cd pv-1.9.34 || { echo "Failure"; exit 1; }
    sh ./configure
    make -j8
